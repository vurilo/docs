...
This guide provides detailed instructions for deploying a vurilo application on a Linux server, including setting up MySQL database, Docker, application server, SSL certificate deployment, and MinIO file server setup. Below is the step-by-step guide:
...
7. Create database:
   ```
   CREATE DATABASE vurilo;
   ```
8. Create user for the database:
   ```
   CREATE USER 'vurilo'@'%' IDENTIFIED BY 'password';
   ```
9. Grant privileges to the user:
   ```
   GRANT ALL PRIVILEGES ON vurilo.* TO 'vurilo'@'%';
   ```
10. Flush privileges:
    ```
    FLUSH PRIVILEGES;
...
1. Login to the MinIO server using the above credentials and create a bucket named `vurilo`.
![Bucket Creation](images/bucket_create.png)
...
"Resource": [
"arn:aws:s3:::vurilo/public/*"
]
}
]
}
```
3. Create `public` and `private` paths in the `vurilo` bucket for storing files.
4. MinIO setup is complete.
5. After setting up the file server, add the access key and the server URL in the application file storage section. To add the configuration, you need to login to the application and navigate to the setings>Admin>filestorage.

This guide provides a comprehensive approach to deploying a vurilo application on a Linux server with MySQL database, Docker, SSL certificate, and MinIO file server setup. Make sure to follow each step carefully for successful deployment.