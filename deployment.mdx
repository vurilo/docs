...
This guide provides detailed instructions for deploying a LMS application on a Linux server, including setting up MySQL database, Docker, application server, SSL certificate deployment, and MinIO file server setup. Below is the step-by-step guide:
...
1. Login to the MinIO server and create a bucket named `LMS`.
2. After creating open detail of that bucket and click on access policy. Select cusom and write below policy:
```
{
"Version": "2012-10-17",
"Statement": [
...
"Resource": [
"arn:aws:s3:::LMS/public/*"
]
}
]
}
```
3. Create `public` and `private` paths in the `LMS` bucket for storing files.
4. MinIO setup is complete.
5. After seting up the file server, add the access key and the server URL in the application file storage section. To add the configuration, you need to login to the application and navigate to the setings>Admin>filestorage.

This guide provides a comprehensive approach to deploying a LMS application on a Linux server with MySQL database, Docker, SSL certificate, and MinIO file server setup. Make sure to follow each step carefully for successful deployment.