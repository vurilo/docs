...
1. **Pull Docker Image:**
   ...
   ```
   sudo docker pull academykit/academy:latest
   ```
...
3. **Start Docker Container:**
   ...
   ```
   sudo docker run -d --name lms_container -p 80:80 academykit/academy:latest
   ```
   Replace `lms_container` with a name for your container.
