...
If you have just installed the academyKit, the default super admin credentials are as below:

- **Email:** academykit@yopmail.com
- **Password:** P@ssw0rd

<Warning>Please change this credentials as soon as possible.</Warning>
...
## Changing Login Credentials for Super Admin

It is crucial to change the initial login credentials, particularly for the super admin account. Failure to update these credentials may result in missing important emails regarding updates and application logs. Follow these steps to update the login credentials and associated email address for the user:

1. Log in using the initial credentials provided.
1. Navigate to the settings section within the application.
1. Locate the account settings.
1. Follow the prompts to update the login credentials and email address for the user account.

<Note>
  Ensure SMTP configuration is completed before changing the email address, as
  failure to do so will result in emails not being sent.
</Note>
